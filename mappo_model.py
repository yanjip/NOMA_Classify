...
class Actor_MLP(nn.Module):
    def __init__(self, args, actor_input_dim):
        ...
        self.log_std = nn.Parameter(torch.zeros([10, args.action_dim]).view(-1, args.action_dim))
...
class MAPPO_agent:
    ...
    def choose_action(self, obs_n, evaluate=False):
        with torch.no_grad():
            ...
            mu, std, log_std  = self.actor(actor_inputs)  # prob.shape=(N,action_dim)
            noise = torch.FloatTensor(np.random.normal(0, 1, size=(std.size()))).to(self.device)
            pi = mu.to(self.device) + noise * std.to(self.device)
            pi = pi.clamp(-1, 1)
            parameter_action = (pi + 1) / 2
            if evaluate:  # When evaluating the policy, we select the action with the highest probability
                ...
                logp = gaussian_likelihood(pi, mu.to(self.device), log_std.to(self.device))
                # return parameter_action.cpu().data.numpy().flatten(), pi.cpu().data.numpy().flatten(), logp.cpu().data.numpy().flatten()
                return parameter_action.cpu().data.numpy(), pi.cpu().data.numpy(), logp.cpu().data.numpy()
            ...
            for index in BatchSampler(SequentialSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                if self.use_rnn:
                    ...
                    for t in range(self.episode_limit):
                        mu, std, parameter_log_std = self.actor(actor_inputs[index, t].reshape(self.mini_batch_size * self.N, -1)) # prob.shape=(mini_batch_size*N, action_dim)
                        parameter_logp = gaussian_likelihood(actor_inputs[index, t].reshape(self.mini_batch_size * self.N, -1), mu, parameter_log_std)
