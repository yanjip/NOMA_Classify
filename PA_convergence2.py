...
# 梯度投影算法
def gradient_projection_algorithm(grad, p0, lower_bounds, upper_bounds, H, alpha=0.00210, iterations=2000):
    p = p0
...
if __name__ == '__main__':
    ...
    lower_bounds = np.array([0.012] * para.K)
    ...
    for i,p in enumerate(ps):
        ...
        print(res1,"----------")
        if len(res1) > 200:
            a = res1[:200]  # 截断到前10个元素
        else:
            a = res1 + [x] * (200 - len(res1))  # 补充零到10个元素
        res[i,: ]=np.array(a)

    # res=np.array(compare(p02))
    # print(res)

    plot_BCD_convergence2(res)
