...
class Actor_Beta(nn.Module):
    def __init__(self, args):
        ...
        self.max_action = para.max_action
        self.fc1 = nn.Linear(para.state_dim, args.hidden_width)
    ...
    def get_dist(self, s):
        ...
        alpha = torch.where(torch.isnan(alpha), torch.full_like(alpha, 1e-6), alpha)
        beta = torch.where(torch.isnan(beta), torch.full_like(beta, 1e-6), beta)
...
class Actor_Gaussian(nn.Module):
    ...
    def forward(self, s):
        s = self.activate_func(self.fc1(s))
        s = self.activate_func(self.fc2(s))
        mean = self.max_action * (1+torch.tanh(self.mean_layer(s))) / 2  # [-1,1]->[-max_action,max_action]
        # mean = self.max_action * (torch.tanh(self.mean_layer(s)))  # [-1,1]->[-max_action,max_action]

        return mean

    def get_dist(self, s): #生成一个正态分布
        mean = self.forward(s)
